...
public class LargeBasinRenderer extends SmartBlockEntityRenderer<LargeBasinBlockEntity> {
    public LargeBasinRenderer(BlockEntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    protected void renderSafe(LargeBasinBlockEntity basin, float partialTicks, PoseStack ms, MultiBufferSource buffer,
        ...
        TransformStack.of(ms)
                .rotateYDegrees(basin.getIngredientRotation().getValue(partialTicks));

        RandomSource r = RandomSource.create(pos.hashCode());
        Vec3 baseVector = new Vec3(.125, level, 0);

        IItemHandlerModifiable inv = basin.getItemCapability();
        if (inv == null)
        ...
        if (!(blockState.getBlock() instanceof LargeBasinBlock))
            return;
        Direction direction = blockState.getValue(LargeBasinBlock.FACING);
        ...
        boolean outToBasin = basin.getLevel()
                .getBlockState(basin.getBlockPos()
                        .relative(direction))
                .getBlock() instanceof LargeBasinBlock;

        for (IntAttached<ItemStack> intAttached : basin.getVisualizedOutputItems()) {
            float progress = 1 - (intAttached.getFirst() - partialTicks) / LargeBasinBlockEntity.OUTPUT_ANIMATION_TIME;
    ...
    protected float renderFluids(LargeBasinBlockEntity basin, float partialTicks, PoseStack ms, MultiBufferSource buffer,
                                 int light, int overlay) {
